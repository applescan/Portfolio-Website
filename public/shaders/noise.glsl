...
float snoise3(vec3 v) {
  ...
  vec4 p = permute3(
              permute3(
                permute3(i.z + vec4(0.0, i1.z, i2.z, 1.0)) +
...
float snoise4(vec4 v) {
  ...
  vec4 j1 = permute4(permute4(permute4(permute4(i.w + vec4(i1.w, i2.w, i3.w, 1.0))
  ...
  vec4 norm = taylorInvSqrt4(vec4(dot(p0, p0), dot(p1, p1),
                                  dot(p2, p2), dot(p3, p3)));
  p0 *= norm.x;
  p1 *= norm.y;
  p2 *= norm.z;
